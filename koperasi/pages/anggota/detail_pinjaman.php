...
if (!$pinjamanId) {
    ...
    redirect('/pages/anggota/pinjaman.php');
}
...
try {
    ...
    ");
    $stmt->execute([$pinjamanId, $user['id']]);
    $pinjaman = $stmt->fetch();
    ...
    // Hitung statistik cicilan
    $totalCicilan = count($cicilan);
    $cicilanDibayar = 0;
    $totalDibayar = 0;
    $sisaPinjaman = $pinjaman['total_bayar'];

    foreach ($cicilan as $c) {
        if ($c['status'] === 'dibayar') {
            $cicilanDibayar++;
            $totalDibayar += $c['jumlah_cicilan'];
            $sisaPinjaman -= $c['jumlah_cicilan'];
        }
    }
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg p-6">
            <h3 class="text-xl font-semibold text-gold mb-4">Informasi Pinjaman</h3>
            <div class="space-y-4">
                <div>
        ...
        <!-- Progress Pembayaran -->
        <div class="bg-gray-800 rounded-lg shadow-lg p-6">
            <h3 class="text-xl font-semibold text-gold mb-4">Progress Pembayaran</h3>
            <div class="space-y-4">
                <div>
                    <label class="block text-sm text-gray-400">Cicilan Dibayar</label>
                    <p class="text-white"><?= $cicilanDibayar ?> dari <?= $totalCicilan ?> cicilan</p>
                    <div class="w-full bg-gray-700 rounded-full h-2.5 mt-2">
                        <div class="bg-gold h-2.5 rounded-full" 
                             style="width: <?= ($cicilanDibayar / $totalCicilan) * 100 ?>%"></div>
                    </div>
                </div>
                <div>
                    <label class="block text-sm text-gray-400">Total Dibayar</label>
                    <p class="text-white">Rp <?= number_format($totalDibayar, 0, ',', '.') ?></p>
                </div>
                <div>
                    <label class="block text-sm text-gray-400">Sisa Pinjaman</label>
                    <p class="text-white">Rp <?= number_format($sisaPinjaman, 0, ',', '.') ?></p>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <tr class="border-t border-gray-700">
                        ...
                        <td class="p-2">
                            <?php if ($c['status'] === 'belum_bayar'): ?>
                                <a href="bayar_cicilan.php?id=<?= $c['id'] ?>" 
